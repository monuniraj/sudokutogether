...
if (best < lines.length) {
  const next = lines[best];
  console.log('next line', best + 1, next);
  const { diags, sf } = parseLines(best + 1);
  console.log('diags', diags.map(d => ({ message: d.messageText, start: d.start, pos: ts.getLineAndCharacterOfPosition(sf, d.start) })));
}
